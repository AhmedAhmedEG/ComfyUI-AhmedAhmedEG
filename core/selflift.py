...
def sample_selflift_progressive(
    ...
    audio_stream = streams[1] if len(streams) > 1 else torch.zeros((1, 32, 2, max(1, round(video_stream.shape[2] * 40 / 24))), device=video_stream.device, dtype=video_stream.dtype)

    orig_h, orig_w = video_stream.shape[-2], video_stream.shape[-1]
    low_h = max(2, (int(round(orig_h * lowres_scale)) // 2) * 2)
    low_w = max(2, (int(round(orig_w * lowres_scale)) // 2) * 2)
    ...
    inter_streams, _ = unpack_av_samples(intermediate["samples"])
    inter_video = inter_streams[0]
    inter_audio = inter_streams[1] if len(inter_streams) > 1 else audio_stream
