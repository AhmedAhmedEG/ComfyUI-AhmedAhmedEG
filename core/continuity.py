...
def extract_streams_from_av_latent(latent_dict: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if hasattr(samples, "unbind"):
        ...
        raise ValueError(f"Expected NestedTensor or tuple of streams, got {type(samples)}")

    if len(streams) < 2:
        raise ValueError(f"AV latent must contain both video and audio streams (got {len(streams)}).")
