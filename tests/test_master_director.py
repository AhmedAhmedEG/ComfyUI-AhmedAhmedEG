...
class TestMiniMaxH3MasterDirector(unittest.TestCase):
    ...
    def test_audio_post_crossfade(self):
        ...
        clip1 = {"waveform": torch.ones((2, 32000), dtype=torch.float32) * 0.5, "sample_rate": 32000}
        clip2 = {"waveform": torch.ones((2, 48000), dtype=torch.float32) * 0.5, "sample_rate": 48000}

        faded = apply_audio_fade(clip2["waveform"], 48000, fade_in_ms=10.0, fade_out_ms=10.0)
        self.assertEqual(faded.shape, clip2["waveform"].shape)
        ...
        self.assertEqual(merged["sample_rate"], 48000)
    ...
    def test_check_lazy_status(self):
        """Verify check_lazy_status returns ['model'] when model is un-evaluated, and [] when evaluated."""
        from nodes.node_director import MiniMaxH3MasterDirector
        self.assertEqual(MiniMaxH3MasterDirector.check_lazy_status(model=object()), [])
        self.assertEqual(MiniMaxH3MasterDirector.check_lazy_status(model=None), ["model"])
